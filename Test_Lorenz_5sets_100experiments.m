...
p = 4; % maximal degree of monomials
...
indexTrue = [2,3,7];
nbar = nchoosek(3+p,3); % number of basis terms
C_true = zeros(nbar,m);
...
error_rel = zeros(maxIter,5);% relative error in the recovered coefficient vector for all 5 bifurcation parameters

count =0; % count the recovery results over maxIter experiments
for iter=1:maxIter
    % Generate noisy data
...
fprintf(['The group hard-iterative algorithm correctly identifies all terms in the governing equation ',num2str(count),' times over ',num2str(maxIter),'\n'])
% Compute Error Relative Ranges
tmp = error_rel;
tmp(error_rel==0) = NaN;
error_min = min(tmp,[],1);
error_max = max(error_rel,[],1);

fprintf('Relative Error Range for each bifurcation parameter is\n')
[error_min;error_max]

%% Visualization of a input data
